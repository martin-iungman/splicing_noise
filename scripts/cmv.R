...
cmv2<-map2(cmv2,names(cmv2), ~.x%>%mutate(ratio=`FL1-H`/(`FL1-H`+`FL2-H`),prom=.y))
cmv_df<-cmv2%>%list_rbind()
ggplot(cmv_df, aes(ratio,fill=prom))+geom_density(alpha=0.7)
...
cmv_df%>%group_by(prom)%>%summarise(mad=mad(ratio),sd=sd(ratio))